  ...
  function kanbanerySiteUrl(url){
    return 'https://' + workspaceName() + '.' + KANBANERY_SITE + '/' + url;
  }
...
Kanbanery.Workspaces = Class.$extend({
  ...
  loadExtraData: function(){
    if (projectId() !== undefined){
      ...
      Kanbanery.ProjectLog.getProjectLog();
    }
  },
...
Kanbanery.MustacheTemplates = {
  projectLogRow: '<li><div class="description">{{description}}</div><div class="info"><span class="date">{{date}}</date><a href="{{link}}">link</a></div></li>'
};


Kanbanery.ProjectLog = {

  getProjectLog: function(){
    var that = this;
    var url = 'projects/' + projectId() + '/log.rss';
    Application.Flash.showNotice("Loading Data");   
    Application.Ajax._get(kanbanerySiteUrl(url), {
      success: function(result){
        Kanbanery.ProjectLog.showProjectLog(result);
      }
    });
  },

  showProjectLog: function(projectLog){
    var projectLogContainer = $('#project-log');

    projectLogContainer.empty();


    _($(projectLog).find('item')).each(function(item){
      ...
      projectLogContainer.append(Mustache.to_html(Kanbanery.MustacheTemplates.projectLogRow, row));
...
Kanbanery.Tabs = {
  init: function(){
    ...
    tabMenu.find('a').click(function(){
      ...
      var tabEl = $(this);
      tabEl.parent().attr('class', 'active');
